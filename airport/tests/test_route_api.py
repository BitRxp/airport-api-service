...
def sample_route(**params):
    ...
    defaults = {
        ...
        "destination": sample_airport(
            name="Sample_name_1",
            closest_big_city="Sample_city_1"
        ),
        "distance": 1000,
    }
...
class AuthenticatedRouteAPITests(TestCase):
    ...
    def test_route_list_with_filters(self):
        ...
        source = sample_airport(name="Sample_name_2",
                                closest_big_city="Sample_city_2")
        destination = sample_airport(
            name="Sample_name_3",
            closest_big_city="Sample_city_3"
        )
        sample_route(
            source=source,
        ...
        routes = Route.objects.filter(
            source__id=source.id, destination__id=destination.id
        )
        serializer = RouteListSerializer(routes, many=True)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, serializer.data)
...
class AdminRouteAPITests(TestCase):
    ...
    def test_route_update_not_allowed(self):
        ...
        source = sample_airport(
            ...
            closest_big_city="Sample_city_2")
        destination = sample_airport(
            name="Sample_name_3", closest_big_city="Sample_city_3"
        )
        data = {
            "source": source.id,
            "destination": destination.id,
