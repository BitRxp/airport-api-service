...
class AuthenticatedFlightAPITests(TestCase):
    ...
    def test_flight_list_with_filters(self):
        ...
        response_source = self.client.get(
            FLIGHT_LIST_URL, {"source": str(airport_source.id)}
        )
        flights_source = Flight.objects.filter(
            route__source__id=airport_source.id
        )
        serializer_source = FlightListSerializer(
            flights_source,
            many=True)
        self.assertEqual(response_source.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response_source.data["results"],
            serializer_source.data
        )

        response_destination = self.client.get(
            FLIGHT_LIST_URL, {"destination": str(airport_destination.id)}
        )
        flights_destination = Flight.objects.filter(
            route__destination__id=airport_destination.id
        )
        serializer_destination = FlightListSerializer(
            flights_destination,
            many=True
        )
        self.assertEqual(response_destination.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response_destination.data["results"], serializer_destination.data
        )
...
class AdminFlightAPITests(TestCase):
    ...
    def test_flight_create(self):
        ...
        self.assertEqual(data["departure_time"], flight.departure_time)
        self.assertEqual(data["arrival_time"], flight.arrival_time)
    ...
    def test_create_departure_time_exceeds_24_hours_after_previous_arrival_forbidden(
        self,
    ):
        flight_1 = sample_flight()
        crew_member = Crew.objects.create(first_name="John", last_name="Doe")
